            ...
            <div className="relative rounded-[var(--radius-lg)] bg-[var(--surface-1)] border border-[var(--border-subtle)] p-3 mb-3">
                ...
                <div className="flex items-center justify-between mb-2.5">
                    <div className="flex items-center gap-2">
                        ...
                        {item.materialId && (
                            <span className="text-[11px] font-medium text-[var(--chart-2)]">
                                {item.codigoInterno}
                            </span>
                        )}
                    </div>
                ...
                <div ref={wrapperRef} className="relative mb-3">
                    {item.materialId ? (
                        <SelectedMaterial item={item} onClear={handleClearMaterial} />
                    ) : (
                        <SearchInput
                            index={index}
                            term={term}
...
function SelectedMaterial({ item, onClear }) {
    return (
        <div className="flex items-center gap-2.5 min-h-[44px] px-3 py-2 rounded-[var(--radius-md)] bg-[color-mix(in_hsl,var(--chart-2)_8%,transparent)] border border-[color-mix(in_hsl,var(--chart-2)_25%,transparent)]">
            <Package className="w-4 h-4 text-[var(--chart-2)] shrink-0" />
...
function SearchDropdown({ results, term, loading, onSelect, onCreateNew }) {
    if (term.length >= 2 && !loading && results.length === 0) {
        return (
            <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[var(--radius-lg)] shadow-xl overflow-hidden">
                <div className="px-4 py-3 text-sm text-[var(--fg-subtle)] border-b border-[var(--border-subtle)]">
                    No se encontró "{term}"
                </div>
                <button
                    onClick={onCreateNew}
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-foreground">Crear nuevo material</div>
                        <div className="text-xs text-[var(--fg-subtle)]">Agregar a la base de datos</div>
                    </div>
                </button>
            </div>
        );
