 ...
 */
/**
 * Si esta orden espera la firma de esta persona.
 *
 * Mira las dos formas en que el tablero puede decir quien aprueba: el usuario
 * de MONDAY de siempre (`aprobadorIds`) y el item de la persona en "Equipo VDV",
 * que el servidor ya tradujo a mail (`aprobadorMails`, ver
 * lib/server/oc-tracker-snapshot.js).
 *
 * Con que coincida CUALQUIERA alcanza. La primera sigue funcionando igual que
 * siempre, asi que a nadie le desaparecen las ordenes que hoy ve; la segunda
 * existe para el dia que VDV borre esos usuarios de monday y el id deje de
 * existir. Si no, la bandeja quedaria vacia para todos.
 */
function esMiOrden(oc, sesionOc) {
  if ((oc.aprobadorIds ?? []).includes(String(sesionOc.mondayUserId))) return true;
  const miMail = String(sesionOc.email ?? "").trim().toLowerCase();
  return Boolean(miMail) && (oc.aprobadorMails ?? []).includes(miMail);
}

export function puedeDeberOc(sesionOc) {
  if (!sesionOc) return false;
  // Antes exigia el id de monday. Ahora alcanza con el mail, que es por donde
  // la bandeja cruza desde el corte (ver esMiOrden y aprobadorMails): quien no
  // tiene licencia igual tiene que ver lo que espera su firma.
  const identificado =
    Number(sesionOc.mondayUserId) > 0 || Boolean(String(sesionOc.email ?? "").trim());
...
export function pendientesDeOc(ordenes, sesionOc) {
  ...
  for (const oc of ordenes ?? []) {
    ...
    if (!esMiOrden(oc, sesionOc)) continue;
...
export function ocSinAprobador(ordenes, sesionOc) {
  ...
  if (!puede) return 0;
  // Huerfana es la que no tiene aprobador POR NINGUNA DE LAS DOS VIAS. Mirar
  // solo `aprobadorIds` empezaria a contar como huerfanas las que tienen el
  // vinculo puesto el dia que se deje de escribir la columna de PERSONA.
  return (ordenes ?? []).filter(
    (oc) =>
      String(oc.estadoDocumento ?? "").trim().toUpperCase() === "PENDIENTE" &&
      (oc.aprobadorIds ?? []).length === 0 &&
      (oc.aprobadorMails ?? []).length === 0,
  ).length;
}
