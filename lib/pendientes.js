...
import { puedeAprobarOc } from "@/lib/oc-roles";
...
export function contratosEsperandoFirma(contratos, sesionPortal) {
  ...
  for (const contrato of contratos ?? []) {
    ...
    if (mios.length === 0) continue;
    // Si todavia le debo un VB, va en el grupo de arriba, no aca.
    if (mios.some((paso) => !yaAprobado(contrato, paso))) continue;
 ...
 *
 * Solo le sirve a quien puede aprobar cualquier orden: el resto no podria
 * hacer nada con ese dato.
 */
export function ocSinAprobador(ordenes, sesionOc) {
  if (sesionOc?.apruebaCualquierOrden !== true) return 0;
