...
/**
 * Tope para la descripcion completa, la que va en la columna aparte.
 *
 * monday acepto 5.000 caracteres en una prueba contra la cuenta real, asi que
 * no es un limite suyo: es nuestro, para que un pegado accidental de veinte
 * paginas no termine en el PDF de un proveedor.
 */
export const MAX_DESCRIPCION = 2000;

/**
 * El nombre que se manda a monday.
 *
 * Si la linea entra, va tal cual. Si no, la descripcion se corta y se marca con
 * un puntito suspensivo, para que quien mire el tablero sepa que hay mas y
 * busque en la columna DESCRIPCIÓN COMPLETA. Cortar es seguro: los numeros van
 * despues del separador y no se tocan.
 */
export function nombreParaMonday(linea, moneda) {
  const nombre = codificarLinea(linea, moneda);
  if (nombre.length <= MAX_NOMBRE_MONDAY) return nombre;

  const tope = maxDescripcion(linea, moneda);
  const cortada = String(linea?.descripcion ?? "").trim().slice(0, Math.max(0, tope - 1)) + "…";
  const recortado = codificarLinea({ ...linea, descripcion: cortada }, moneda);
  // Red de seguridad para una linea absurda (numeros gigantes): antes que
  // perderla, se manda cortada a lo bruto. Un nombre feo se arregla; una linea
  // que monday rechaza desaparece del detalle sin que nadie se entere.
  return recortado.length <= MAX_NOMBRE_MONDAY ? recortado : recortado.slice(0, MAX_NOMBRE_MONDAY);
}

/** Si esta linea necesita que la descripcion se guarde ademas en su columna. */
export function necesitaDescripcionAparte(linea, moneda) {
  return excedeNombre(linea, moneda) > 0;
}

/**
 * La descripcion de verdad de un subelemento: la columna cuando tiene algo, y
 * si no, la parte del nombre.
 *
 * Con esta regla las lineas viejas -que solo tienen el nombre- y las nuevas
 * conviven sin ningun caso especial, y no hay que migrar las que ya estan.
 */
export function descripcionDeSubitem(sub) {
  const completa = String(sub?.descripcionCompleta ?? "").trim();
  if (completa) return completa;
  return decodificarLinea(sub?.name ?? "")?.descripcion ?? "";
}
