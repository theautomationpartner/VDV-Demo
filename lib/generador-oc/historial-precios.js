...
import { decodificarLinea, descripcionDeSubitem, precioFinal } from "./linea-oc";
import { normalizarMaterial, normalizarUnidad } from "./normalizacion-material";
import { compararMateriales, tokenPrincipal } from "./match-materiales";
...
export function lineasDeOcEmitidas() {
  return conCache("historialLineasOc", async () => {
    const res = await ocBoard
      ...
      .withSubItems("SubelementosDeOrdenesDeCompraMaxxaBoard", ["descripcionCompleta"])
      .withRelations()
    ...
    for (const oc of res?.items ?? []) {
      ...
      (oc.subitems ?? []).forEach((sub, idx) => {
        ...
        if (!linea) return;
        // El nombre puede venir cortado; el historial compara materiales por
        // descripcion, asi que tiene que usar la de verdad.
        linea.descripcion = descripcionDeSubitem(sub);
...
export async function consultarPrecioMaterial({ nombre, unidad, moneda, precioActual }) {
  ...
  try {
    ...
    for (const fila of todas) {
      ...
      // Prefiltro barato para acotar candidatos. NO decide nada: la semilla es
      // la palabra mas larga del nombre, asi que "TORNILLO CRS 8 X 2 1/2" trae
      // todos los tornillos del historial.
      if (!normHist.includes(semilla) && !normalizado.includes(tokenPrincipal(fila.nombre))) continue;

      // El juicio de verdad. Sin esto la alerta comparaba un TORNILLO CRS 8 X 2
      // 1/2" contra un TORNILLO P/FINA 6X1 1/4 y avisaba "+83,8% sobre el mejor
      // precio reciente" sobre dos productos distintos.
      const match = compararMateriales(nombre, fila.nombre);
      if (match.nivel === "NO_COMPARABLE") continue;
      ...
      // Una coincidencia POSIBLE no alcanza para afirmar "estas pagando de
      // mas": es el mismo criterio que ya aplica evaluarComparacion, que marca
      // `comparable: false` cuando el nivel es dudoso. Igual aparece en la
      // ficha del material, que la muestra aparte y rotulada.
      if (match.nivel === "POSIBLE") continue;

      comparables.push({ ...fila, nivel: match.nivel });
    }
...
export async function historialDeMaterial({ nombre, moneda }) {
  ...
  try {
    ...
    return todas
      .filter((f) => {
        // Mismo prefiltro barato que en consultarPrecioMaterial: acota, no decide.
      ...
      .map((f) => ({ ...f, nivel: compararMateriales(nombre, f.nombre).nivel }))
      // Aca SI se conservan las POSIBLE: el panel las muestra aparte y
      // rotuladas (ver HistorialMaterialPanel, que calcula los KPI solo con las
      // seguras, y TablaComprasMaterial, que les pone el distintivo).
      .filter((f) => f.nivel !== "NO_COMPARABLE")
      .sort(porFechaDesc);
  } catch (error) {
