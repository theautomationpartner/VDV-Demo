...
/**
 * El proximo numero de OC: el mayor que exista en el tablero, mas uno.
 *
 * Recorre todas las ordenes a proposito. NUMERO OC es una columna de texto y no
 * hay nada en monday que garantice el orden ni la continuidad, asi que mirar
 * solo la ultima creada daria un numero repetido en cuanto alguien cargue una
 * orden a mano fuera de la app.
 */
export async function getNextOcNumber() {
  try {
    const items = await fetchAllItems(ocBoard.items().withColumns(["numeroOc"]));
    let maximo = 0;
    for (const item of items) {
      const numero = parseInt(String(item.numeroOc ?? "").trim(), 10);
...
export function getObrasOc() {
  return ocBoard.columnOptions("obra");
}

export function getCondicionesOc() {
  return ocBoard.columnOptions("condicionDeCompra");
}
...
export function getBancoOptions() {
  return proveedoresBoard.columnOptions("banco").catch(() => []);
}
...
/**
 * Usuarios de monday que pueden quedar como aprobadores. Se excluye al emisor:
 * nadie aprueba su propia orden.
 */
export async function getUsuariosAprobadores(excluirId) {
  try {
    const usuarios = await ocBoard.users.withPagination({ limit: 200 }).execute();
    return (usuarios ?? [])
      .map((u) => ({
        id: Number(u.id ?? 0),
        name: u.name ?? "",
