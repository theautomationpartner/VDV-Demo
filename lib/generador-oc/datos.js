...
/**
 * Si otra orden se quedo con este numero, devuelve su id.
 *
 * getNextOcNumber lee el maximo del tablero y despues se crea el item: entre
 * esas dos cosas hay una ventana, y dos personas emitiendo a la vez sacan el
 * mismo numero. Todavia no paso -en el tramo del Generador, de la 2159 a la
 * 2214, no hay ninguno repetido- pero mas abajo el tablero tiene 53 numeros
 * duplicados de la epoca en que se cargaban a mano, asi que el problema es
 * conocido.
 *
 * Desempate: se queda con el numero el item mas viejo, que en monday es el de
 * id mas chico. Asi las dos partes de la carrera llegan a la misma conclusion
 * sin hablarse.
 */
async function otraOcConEseNumero(numeroOc, miItemId) {
  const { items } = await ocBoard
    .items()
    .withColumns(["numeroOc"])
    .where({ numeroOc: { contains: String(numeroOc) } })
    .withPagination({ limit: 20 })
    .execute();

  return (items ?? []).find(
    (item) =>
      String(item.numeroOc ?? "").trim() === String(numeroOc) &&
      String(item.id) !== String(miItemId) &&
      BigInt(item.id) < BigInt(miItemId),
  );
}
...
export async function createOc(data) {
  ...
  // El numero se toma al crear el item, asi que puede haberse ido mientras
  // tanto. Se comprueba despues de crear y, si se lo llevo otro, se borra este
  // y se vuelve a empezar con el siguiente.
  let numeroOc;
  let codigoValidacion;
  let item;

  for (let intento = 1; intento <= 3; intento++) {
    numeroOc = String(await getNextOcNumber());
    codigoValidacion = calcularCodigoValidacion({
      numeroOc,
      total,
    ...
    try {
      ...
      // aprueba. Si fallan, el servidor ya borro el item a medio crear, asi que
      // el numero no se pierde (ver handleItemCreate).
      throw traducirPersonaInvalida(error);
    }

    let duplicada = null;
    try {
      duplicada = await otraOcConEseNumero(numeroOc, item.id);
    } catch (error) {
      // Si no se puede comprobar, se sigue: una orden emitida vale mas que la
      // certeza de que el numero es unico, y hasta hoy nunca se repitio.
      console.error("[generador-oc] No se pudo verificar el número de la orden:", error);
    }
    if (!duplicada) break;

    console.warn(
      `[generador-oc] El número ${numeroOc} se lo llevó la orden ${duplicada.id}: se reintenta con el siguiente.`,
    );
    await borrarOcIncompleta(item.id).catch((error) =>
      console.error("[generador-oc] No se pudo deshacer la orden duplicada:", error),
    );
    item = null;
  }
  ...
  if (!item) {
    throw new Error(
      "Otra persona está emitiendo órdenes en este momento y el número quedó tomado. Probá de nuevo en un minuto.",
    );
  }
 ...
 */
export async function borrarOcIncompleta(itemId) {
  ...
  for (let intento = 0; intento <= esperas.length; intento++) {
    try {
      return await ocBoard.ocRollback(itemId);
    } catch (error) {
