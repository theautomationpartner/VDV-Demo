...
const COLUMNAS = ["mail", "cargo", "telefono", "estado"];
    ...
    try {
      ...
      for (const item of items ?? []) {
        const persona = {
          ...
          // Solo se usa para NO ofrecer fichas dadas de baja al elegir
          // aprobador. Leer una orden vieja sigue funcionando aunque la ficha
          // este inactiva: si no, se perderia el nombre de quien la firmo.
          activa: String(item.estado ?? "").trim().toUpperCase() !== "INACTIVO",
        };
 ...
 */
/**
 * Las fichas activas del directorio, ordenadas por nombre. Es la lista de la
 * que sale el desplegable de aprobadores.
 */
export async function personasActivas() {
  const { porItem } = await getEquipoVdv();
  return [...porItem.values()]
    .filter((p) => p.activa && p.mail)
