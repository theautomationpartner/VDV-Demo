  ...
  if (!logoEnCurso) {
    logoEnCurso = fetch("/api/generador-oc/logo")
      ...
      .catch((error) => {
        ...
        console.error("[generador-oc] No se pudo cargar el logo para el documento:", error);
        return null;
      })
      .finally(() => {
        logoEnCurso = null;
...
export async function generateOcPdf(data) {
  ...
  if (data.urlValidacion) {
    try {
      ...
      console.error("[generador-oc] No se pudo generar el QR de validación:", error);
    }
  }
                      ...
                      [
                        {
                          ...
                          stack: [
                            ...
                            ...(data.firmaAprobador?.cargo
                              ...
                              : [{ text: "", margin: [8, 0, 8, 2] }]),
                            {
                              // La fecha del aprobador es la de SU firma, no la
                              // de la emision: se aprueba dias despues. Antes
                              // salian las dos iguales.
                              text: data.firmaAprobador?.imagen
                                ? `Firmado electrónicamente el ${
                                    data.firmaAprobador.fechaHora ?? data.firma.fechaHora
                                  }`
                                : "Pendiente de firma",
                              style: "firmaMeta",
                              margin: [8, 0, 8, 8],
