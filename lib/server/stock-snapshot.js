...
/**
 * Cuanto vale un snapshot antes de considerarlo viejo. Es una corrida perdida
 * de margen sobre la tarea programada, que desde el 21-sep-2026 corre cada 30
 * minutos y no cada 5 (ver vercel.json y lib/server/franja-horaria.js).
 */
export const FRESCURA_MS = 35 * 60 * 1000;
...
/**
 * Cuando se recalculo el stock por ultima vez, sin traerse los datos. Lo usa el
 * freno de /recalcular: una sola consulta, en vez de leer un snapshot entero
 * nada mas que para mirarle la fecha.
 *
 * Va por MAX() y no por la fila "*por-material*" para no depender de que esa
 * fila exista: guardarStock la escribe al final, y entre medio puede no estar.
 */
export async function leerMarcaDeStock() {
  await asegurarTabla();
  const filas = await sql`SELECT MAX(calculado_en) AS calculado_en FROM stock_snapshot`;
  const marca = filas[0]?.calculado_en;
