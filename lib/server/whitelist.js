import "server-only";
import { sql } from "@/lib/server/db";

/**
 * Whitelist de emails - es el login entero de la app (link publico, sin iframe
 * de monday.com de por medio: no hay sessionToken del que sacar identidad, asi
 * que la fuente de verdad es directamente el email que la persona escribe).
 *
 * Regla de oro: se consulta en cada intento de login, no se cachea el resultado
 * "autorizado" mas alla de la sesion ya emitida (ver lib/server/session.js) - si
 * se revoca a alguien, el proximo intento de generar sesion falla, aunque su
 * sesion vieja siga viva hasta que expire.
 */

export class NoAutorizado extends Error {}

/**
 * El mismo evento, en una linea y con un solo marcador, para que quede en los
 * logs de Vercel: filtrando `[auth]` en Logs sale la historia completa de quien
 * intento entrar, cuando, desde donde y que paso - sin tener que pedirle nada a
 * la persona ni entrar a la base.
 *
 * Se escribe ANTES de la insercion y fuera de su try: si la base no contesta,
 * el rastro tiene que quedar igual (justamente ahi es cuando mas hace falta).
 *
 * Va por console.log y no console.error a proposito: no todos son errores, y
 * como error ensuciarian el panel de anomalias de Vercel.
 */
function registrarEnLog({ usuarioId, detalle, ...resto }) {
  console.log("[auth]", JSON.stringify({ ...resto, ...(detalle ?? {}) }));
}

async function auditar(evento) {
  registrarEnLog(evento);
  try {
    await sql`
      insert into auditoria (usuario_id, email, accion, ip, detalle)
      values (${evento.usuarioId ?? null}, ${evento.email ?? null}, ${evento.accion}, ${evento.ip ?? null}, ${evento.detalle ?? null})
    `;
  } catch (err) {
    // La auditoria nunca debe tumbar el pedido real.
    console.error("[auditoria] no se pudo registrar:", err.message);
  }
}

/**
 * Verifica que el email este habilitado en la whitelist. Devuelve el registro
 * completo (id, email, rol) o tira NoAutorizado - siempre con el mismo mensaje
 * generico, para no confirmarle a quien esta tanteando si el email existe o no.
 */
export async function verificarEmailEnWhitelist(email, { ip } = {}) {
  const normalizado = email.toLowerCase().trim();
  const rows = await sql`select * from usuarios_autorizados where email = ${normalizado} limit 1`;
  const usuario = rows[0] ?? null;

  if (!usuario || usuario.estado !== "activo") {
    await auditar({ email: normalizado, accion: "no_autorizado", ip });
    throw new NoAutorizado();
  }

  await auditar({ usuarioId: usuario.id, email: usuario.email, accion: "login_intento_ok", ip });
  return usuario;
}

export async function marcarUltimoAcceso(usuarioId) {
  await sql`update usuarios_autorizados set ultimo_acceso = now() where id = ${usuarioId}`;
}

/**
 * `detalle` es lo que despues permite diagnosticar sin adivinar: por que se
 * rechazo un codigo, cuantos segundos corrido estaba, etc. Queda en la columna
 * JSONB de auditoria y, aplanado, en la linea de log.
 */
export async function auditarEvento(usuarioId, email, accion, ip, detalle = null) {
  await auditar({ usuarioId, email, accion, ip, detalle });
}

// --- Administracion de la whitelist (panel de admin, ver app/api/auth/whitelist
// y app/admin/whitelist/page.jsx) ---

export async function listarUsuariosAutorizados() {
  return sql`
    select id, email, nombre, rol, estado, asignaciones, creado_en, ultimo_acceso
    from usuarios_autorizados order by creado_en desc
  `;
}

// Usado por PATCH/DELETE en app/api/auth/whitelist/route.js para chequear
// permisos acotados por app (asignaciones reales, no lo que el cliente vio
// filtrado) antes de aplicar el cambio.
export async function obtenerUsuarioAutorizado(id) {
  const rows = await sql`
    select id, email, nombre, rol, estado, asignaciones from usuarios_autorizados where id = ${id} limit 1
  `;
  return rows[0] ?? null;
}

// asignaciones: array de {app, appRol, appConfig} - la mayoria de la gente
// tiene una sola, pero puede tener varias (ej. un admin de la agencia con
// Super Admin en Vale Express Y Portal Proveedor).
export async function agregarUsuarioAutorizado({ email, nombre, rol = "usuario", asignaciones = [] }) {
  const rows = await sql`
    insert into usuarios_autorizados (email, nombre, rol, asignaciones)
    values (${email.toLowerCase().trim()}, ${nombre ?? null}, ${rol}, ${JSON.stringify(asignaciones)})
    on conflict (email) do update set
      estado = 'activo',
      nombre = excluded.nombre,
      rol = excluded.rol,
      asignaciones = excluded.asignaciones
    returning id, email, nombre, rol, estado, asignaciones
  `;
  return rows[0];
}

export async function actualizarUsuarioAutorizado(id, { nombre, rol, estado, asignaciones }) {
  const rows = await sql`
    update usuarios_autorizados
    set
      nombre = coalesce(${nombre ?? null}, nombre),
      rol = coalesce(${rol ?? null}, rol),
      estado = coalesce(${estado ?? null}, estado),
      asignaciones = coalesce(${asignaciones ? JSON.stringify(asignaciones) : null}, asignaciones)
    where id = ${id}
    returning id, email, nombre, rol, estado, asignaciones
  `;
  return rows[0] ?? null;
}

export async function eliminarUsuarioAutorizado(id) {
  await sql`delete from usuarios_autorizados where id = ${id}`;
}
