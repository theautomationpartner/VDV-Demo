...
// Cada "window" son 30s para cada lado. 1 (+-30s) alcanza para el desfasaje de
// reloj de un celular, que es de lo unico que hay que cubrirse en el login de
// todos los dias: ahi la persona ya tiene la app abierta y copia el codigo al
// momento.
const VENTANA_LOGIN = 1;

// El enrolamiento es otra cosa: hay que salir a la app de autenticacion, cargar
// la cuenta (a veces tipeando la clave a mano), y volver - y en el medio el
// navegador del celular puede descartar la pestaña y obligar a escribir el email
// de nuevo. Medido en produccion: los enrolamientos que fallaban tardaban entre
// 46s y 101s entre pedir el QR y confirmar, o sea que el codigo ya habia vencido
// antes de llegar a apretar el boton; el unico que entro tardo 4s.
const VENTANA_SETUP = 4;

function validarCodigo(secret, code, ventana = VENTANA_LOGIN) {
...
export async function confirmarSetupMfa(usuarioId, code) {
  ...
  const delta = validarCodigo(datos.secret, code, VENTANA_SETUP);
