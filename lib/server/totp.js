...
/**
 * Por que fallo, para poder decirselo en criollo y para dejarlo medido en los
 * logs. Ver VENTANA_DIAGNOSTICO.
 *
 * `desfaseSegundos` es lo que mas sirve para diagnosticar, y hay que leerlo
 * mirando VARIOS intentos seguidos de la misma persona:
 *   - se repite parecido (-240, -240, -240) -> el reloj de su celular esta
 *     corrido eso; cada codigo nuevo que lee ya nace corrido lo mismo.
 *   - va creciendo (-30, -60, -90) -> esta reescribiendo el mismo codigo viejo,
 *     que se pone mas viejo en cada intento.
 */
function motivoDelRechazo(secret, code) {
  const delta = validarCodigo(secret, code, VENTANA_DIAGNOSTICO);
  if (delta === null) return { reason: "codigo_de_otra_cuenta", desfaseSegundos: null };
  ...
  return {
    reason: delta > 0 ? "reloj_adelantado" : "codigo_vencido",
    desfaseSegundos: delta * 30,
  };
}
...
export async function confirmarSetupMfa(usuarioId, code) {
  ...
  if (delta === null) return { ok: false, ...motivoDelRechazo(datos.secret, code) };
