...
// app/app_rol/app_config viajan en el token para no tener que ir a la DB de
// nuevo en cada paso del login (setup/confirm/verify) solo para saber a que
// app pertenece esta cuenta y que rol tiene ahi adentro.
export function datosApp(usuario) {
  // Acepta tanto la fila de la DB (snake_case: app_rol, app_config) como el
  // resultado ya decodificado de verificarPreAuthToken (camelCase: appRol,
  // appConfig) - crearSesion se llama con los dos segun desde donde se invoque.
  return {
    app: usuario.app ?? null,
    appRol: usuario.appRol ?? usuario.app_rol ?? null,
    appConfig: usuario.appConfig ?? usuario.app_config ?? {},
