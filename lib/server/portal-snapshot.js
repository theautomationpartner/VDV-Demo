...
const OC_COLS = [
  ...
  // Quien emitio, apuntando a su ficha en "Equipo VDV". Reemplaza a la columna
  // de PERSONA, que se queda vacia cuando le dan de baja la licencia: el
  // proveedor veria la fila "Responsable" desaparecer de su orden.
  "responsableVdv",
];
...
export async function calcularDatosPortal() {
  ...
  return { pagos, contratos, estadosDePago, ordenes: ordenes.map(responsableDelVinculo), facturas };
}

/**
 * El RESPONSABLE que ve el proveedor sale del vinculo a "Equipo VDV", con la
 * columna de PERSONA de respaldo para cualquier orden que no lo tenga.
 *
 * Se hace aca y no al servir porque el resultado es el mismo para todos y el
 * snapshot se calcula una vez cada media hora, mientras que servirlo pasa en
 * cada pedido. El vinculo no se guarda: solo se usa para esto.
 */
function responsableDelVinculo(orden) {
  const { responsableVdv, ...resto } = orden;
