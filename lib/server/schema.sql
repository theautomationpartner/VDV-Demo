-- Esquema de Capa 2 (whitelist de emails) y Capa 3 (2FA / TOTP) - ver SeguidadApp.md
-- en la raiz del repo original para el diseño completo. Se corre una sola vez con
-- `node scripts/migrate.js` (usa DATABASE_URL). Todas las tablas son IF NOT EXISTS,
-- asi que correrlo de nuevo no rompe nada.

CREATE TABLE IF NOT EXISTS usuarios_autorizados (
  id                SERIAL PRIMARY KEY,
  email             TEXT NOT NULL UNIQUE,
  monday_user_id    BIGINT UNIQUE,          -- se completa al primer ingreso
  monday_account_id BIGINT,
  rol               TEXT NOT NULL DEFAULT 'usuario',  -- 'admin' | 'usuario'
  estado            TEXT NOT NULL DEFAULT 'activo',   -- 'activo' | 'revocado'
  creado_en         TIMESTAMPTZ NOT NULL DEFAULT now(),
  ultimo_acceso     TIMESTAMPTZ
);

CREATE TABLE IF NOT EXISTS mfa_usuarios (
  monday_user_id    BIGINT PRIMARY KEY,
  secreto_cifrado   TEXT NOT NULL,          -- AES-256-GCM, clave en MFA_ENCRYPTION_KEY
  confirmado_en     TIMESTAMPTZ,
  ultimo_periodo    BIGINT,                 -- anti-reutilizacion de codigo TOTP
  creado_en         TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE TABLE IF NOT EXISTS mfa_codigos_recuperacion (
  id                SERIAL PRIMARY KEY,
  monday_user_id    BIGINT NOT NULL,
  hash_codigo       TEXT NOT NULL,          -- sha256, nunca en texto plano
  usado_en          TIMESTAMPTZ
);
CREATE INDEX IF NOT EXISTS idx_mfa_codigos_recuperacion_user ON mfa_codigos_recuperacion (monday_user_id);

CREATE TABLE IF NOT EXISTS dispositivos_confiables (
  id                SERIAL PRIMARY KEY,
  monday_user_id    BIGINT NOT NULL,
  hash_token        TEXT NOT NULL,
  expira_en         TIMESTAMPTZ NOT NULL,   -- +30 dias
  user_agent        TEXT,
  creado_en         TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_dispositivos_confiables_user ON dispositivos_confiables (monday_user_id);

CREATE TABLE IF NOT EXISTS auditoria (
  id                SERIAL PRIMARY KEY,
  monday_user_id    BIGINT,
  email             TEXT,
  accion            TEXT NOT NULL,          -- 'ingreso_ok' | 'no_autorizado' | 'mfa_fallido' | 'mfa_ok' | ...
  ip                TEXT,
  detalle           JSONB,
  creado_en         TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_auditoria_creado_en ON auditoria (creado_en DESC);
