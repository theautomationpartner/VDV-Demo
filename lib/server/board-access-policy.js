...
export async function verificarAccesoMutacion(sesion, boardKey, { op, values, itemId } = {}) {
  ...
  if (boardKey === "OrdenesDeCompraMaxxaBoard") {
    ...
    // Se deja como estaba para no romper una pantalla que hoy funciona - salvo
    // que quien escribe tenga el Generador, en cuyo caso pasa por las reglas
    // de la orden (no puede aprobar una que no le toca).
    const claves = Object.keys(values ?? {});
    const soloEstado = op === "itemUpdate" && claves.length === 1 && claves[0] === "estadoDocumento";
    if (soloEstado && !tieneGeneradorOc(sesion)) return;
