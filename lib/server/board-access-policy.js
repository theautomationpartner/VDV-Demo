import "server-only";
import { itemDeEquipoVdv } from "./equipo-vdv";
...
export const PORTAL_APP = "portal-proveedor";

// Las cinco columnas de VB del circuito de contratos. La pantalla ya controla
// quien puede tocar cual, pero eso es cosmetico: sin este check, cualquier
// sesion valida podia escribir los cinco vistos buenos de cualquier contrato
// llamando a /api/monday/board directo.
const COLUMNAS_VB = PASOS_VB.map((p) => p.campo);
...
/**
 * Quien puede escribir UN visto bueno de UN contrato.
 *
 * Tres condiciones, y las tres se verifican contra el contrato real, no contra
 * lo que mando el navegador:
 *
 *   1. Se toca UNA sola columna de VB, y ninguna otra del contrato. Si el
 ...
 *   2. Esa columna es un paso que la persona tiene asignado PARA LA OBRA de ese
 *      contrato. OT y Administrador cambian de persona segun la obra (se
 *      verifico contra el tablero: la misma cuenta es OT y Administrador en
 *      tres obras y solo OT en FORESTAL), asi que un permiso global alcanzaria
 *      para aprobar donde no corresponde.
 *   3. Los pasos anteriores ya estan en VB. Antes el orden lo respetaba solo la
 *      pantalla; por la API se podia firmar fuera de turno. El super aprobador
 *      es la unica excepcion, y es para lo que existe.
 *
 * Cuesta una consulta a monday por VB escrito. Es el mismo precio que ya paga
 * requireGestionOc, y sin ella los puntos 2 y 3 no se pueden verificar.
 */
async function requireRolContrato(sesion, itemId, values = {}, { esNota = false } = {}) {
  ...
  if (!esSuperAprobador(config) && pasosAsignados(config).length === 0) {
    throw new BoardAccessError("Tu cuenta no tiene asignado ningun paso de aprobacion de contratos.");
  }

  const claves = Object.keys(values ?? {});
  if (claves.length !== 1 || !COLUMNAS_VB.includes(claves[0])) {
    throw new BoardAccessError("Solo podes dar de a un visto bueno por vez.");
  }
  const paso = PASOS_VB.find((p) => p.campo === claves[0]);

  if (!itemId) throw new BoardAccessError("Falta el contrato sobre el que se quiere operar.");

  const col = (clave) => resolveColumnId("FlujoContratacionSubcontratoBoard", clave);
  const ids = [col("obra"), ...COLUMNAS_VB.map(col)];

  let datos;
  try {
    datos = await mondayFetch(
      `query ($itemId: [ID!]) {
        items(ids: $itemId) { column_values(ids: ${JSON.stringify(ids)}) { id text } }
      }`,
      { itemId: [String(itemId)] },
    );
  } catch (error) {
    console.error("[contratos] No se pudo verificar el permiso sobre el contrato:", error?.message);
    throw new BoardAccessError("No se pudo verificar tu permiso sobre este contrato.");
  }

  const item = datos.items?.[0];
  if (!item) throw new BoardAccessError("No se encontro ese contrato.");
  const texto = (columnId) => item.column_values.find((c) => c.id === columnId)?.text ?? "";

  // El contrato en la forma que espera lib/contratos-vb.js: la obra y los cinco
  // VB, con las claves del schema.
  const contrato = Object.fromEntries(PASOS_VB.map((p) => [p.campo, texto(col(p.campo))]));
  const obra = texto(col("obra"));

  if (!pasosEnContrato(config, obra).some((p) => p.paso === paso.paso)) {
    throw new BoardAccessError(
      `No te corresponde el ${paso.label} en esta obra${obra ? ` (${obra})` : ""}.`,
    );
  }

  // Una NOTA no es una aprobacion: llega despues de que la columna ya se
  // escribio, para dejar registrado quien lo hizo. Si se le pidiera el mismo
  // turno se rechazaria siempre -el paso ya figura dado- y la nota se perdia en
  // silencio, que es justo la trazabilidad que el cliente pide. Sigue exigiendo
  // que el paso y la obra sean los tuyos.
  if (esNota) return;

  if (!pasoHabilitado(contrato, paso, config)) {
    throw new BoardAccessError(
      motivoBloqueo(contrato, paso, config) ?? "Todavia no es tu turno en este contrato.",
    );
  }
}
 ...
 *                         aprobador designado de ESA orden, salvo que tenga
 *                         marcada la casilla "aprueba cualquier orden"
...
async function requireGestionOc(sesion, itemId, values = {}) {
  ...
  // Quien sos, por las dos vias. La que vale es la FICHA: el id de monday se
  // conserva solo para reconocer las ordenes viejas, que guardan personas.
  //
  // Antes esto exigia el id de monday y cortaba sin el. Esa exigencia es la
  // que obligaba a cargarle un usuario de monday a cada aprobador en Usuarios
  // y Roles, y a quien no tenia uno propio se le puso el de una cuenta
  // compartida -de ahi las cuatro OC con el aprobador equivocado-.
  const miId = mondayUserIdDeSesion(sesion);
  const miItemVdv = await itemDeEquipoVdv(sesion?.email).catch(() => null);
  if (!miId && !miItemVdv) {
    throw new BoardAccessError(
      "Tu cuenta no figura en el tablero Equipo VDV, asi que no puede gestionar ordenes. " +
        "Pedile a un administrador que te agregue con tu mail.",
    );
  }
  ...
  const ids = [
    ...
    col("aprobador"),
    col("estadoDocumento"),
    // Las mismas dos personas, pero apuntando a su item en "Equipo VDV". Ver
    // mas abajo por que se miran las cuatro columnas y no solo dos.
    col("responsableVdv"),
    col("aprobadorVdv"),
  ];
  ...
  try {
    datos = await mondayFetch(
      // En una columna de conexion `value` SIEMPRE viene null: lo vinculado
      // vive en `linked_item_ids` y hay que pedirlo con el fragmento del tipo.
      `query ($itemId: [ID!]) {
        items(ids: $itemId) {
  ...
  /**
   * Quien sos vos para ESTA orden, mirando las dos formas a la vez.
   *
   * La columna de PERSONA guarda un usuario de monday; la de CONEXION, un item
   * del tablero "Equipo VDV". Hoy dicen lo mismo, pero el dia que VDV borre los
   * usuarios para bajar el plan la primera se queda sin con que comparar y
   * nadie podria aprobar nada.
   *
   * Alcanza con coincidir en CUALQUIERA de las dos, a proposito: asi nadie que
   * hoy entra se queda afuera -la regla vieja sigue intacta y esto se suma al
   * lado-. Cuando la columna de PERSONA se borre, va a quedar sola la nueva sin
   * que haya que tocar nada mas.
   *
   * Que sea mas permisivo no abre nada: para aprovecharlo habria que editar a
   * mano la columna de conexion en monday, y quien puede hacer eso ya puede
   * editar la de PERSONA igual.
   */
  const estoyVinculado = (clave) => {
    if (!miItemVdv) return false;
    const vinculados = valor(col(clave))?.linked_item_ids ?? [];
    return vinculados.map(String).includes(String(miItemVdv));
  };

  const soyResponsable =
    (miId && responsables.includes(miId)) || estoyVinculado("responsableVdv");
  const soyAprobador = (miId && aprobadores.includes(miId)) || estoyVinculado("aprobadorVdv");

  // Antes esto salia del cargo en el perfil de monday ("Gerente General").
  // Ahora es una casilla en Usuarios y Roles - ver apruebaCualquierOrden.
  const apruebaTodo = apruebaCualquierOrden(asignacion);
  ...
  if (quiereAprobar) {
    if (!puedeAprobarOc(asignacion.appRol)) {
      throw new BoardAccessError(
        `Tu rol en el OC Tracker (${etiquetaRolOc(asignacion.appRol)}) no puede aprobar ordenes.`,
      );
    }
    // Nadie firma las dos puntas de su propia orden. El comodin
    // "Puede aprobar cualquier orden" es la unica excepcion, y es explicita:
    // se tilda por persona en Usuarios y Roles.
    //
    // Hasta ahora esto solo lo cuidaba la pantalla, sacando al emisor de la
    // lista de aprobadores. Dejo de funcionar con el corte de las columnas de
    // PERSONA -el id del emisor quedaba en 0 y el filtro no excluia a nadie-,
    // asi que cualquiera podia editar su orden, designarse a si mismo y
    // firmarla. El control tiene que estar aca, no en la pantalla.
    if (soyResponsable && !apruebaTodo) {
      throw new BoardAccessError(
        "No podes aprobar una orden que emitiste vos. Tiene que aprobarla otra persona.",
      );
    }
    if (!soyAprobador && !apruebaTodo) {
      throw new BoardAccessError("Solo la persona asignada como aprobador puede aprobar esta orden.");
    }
    return;
  }

  if (!soyResponsable && !soyAprobador && !apruebaTodo) {
...
const TABLEROS_DEL_OC_TRACKER = new Set([
  ...
  "FacturasIaBoard",
  // "Equipo VDV" no es del Tracker, pero se guarda con la misma llave: tiene el
  // mail, el telefono y el cargo de todo el equipo. Si quedara con la regla por
  // defecto -abierto a cualquier sesion valida- un subcontratista del Portal se
  // bajaria esa lista entera por /api/monday/board. Lo lee el Generador de OC
  // para vincular quien emite y quien aprueba, asi que pedir el Generador
  // alcanza. Mismo criterio que cerramos el 1-sep con los otros tres.
  "EquipoVdvBoard",
]);
 ...
 */
export async function verificarAccesoMutacion(sesion, boardKey, { op, values, itemId, esNota = false } = {}) {
  ...
  if (boardKey === "FlujoContratacionSubcontratoBoard") {
    ...
    await requireRolContrato(sesion, itemId, values, { esNota });
    return;
  }
...
const COLUMNAS_UPLOAD_PERMITIDAS = {
  IngresosBoard: ["foto"],
  // docOc: el PDF de la orden. firmaEmisor: el dibujo de quien la emitio, que
  // se guarda para poder estamparlo de nuevo cuando el aprobador firme y el
  // documento se regenere. Las dos las escribe la misma persona en el mismo
  // momento y con el mismo permiso, asi que no hace falta separarlas.
  // docOcEmision guarda la copia del PDF tal como se emitio: se sube en el
  // mismo momento y con el mismo permiso que docOc.
  OrdenesDeCompraMaxxaBoard: ["docOc", "firmaEmisor", "docOcEmision"],
};
...
export function verificarAccesoUpload(sesion, boardKey, { columnId } = {}) {
  ...
  const permitidas = (COLUMNAS_UPLOAD_PERMITIDAS[boardKey] ?? []).map((clave) =>
    resolveColumnId(boardKey, clave),
  );
  if (!permitidas.includes(columnId)) {
 ...
 *
 * La app lo usa en dos lugares del Portal, los dos sobre algo que le pertenece
 * al proveedor: "Descargar contrato firmado" (ContractDetail.jsx) y "Descargar
 * orden de compra" (OCDetail.jsx). Cualquier otro tablero se rechaza.
 *
 * Ademas verifica que ESE item sea del proveedor que lo pide. Antes no lo
 ...
 *
 * En las ORDENES hay un control mas: solo se entrega el documento de una orden
 * APROBADA. El requerimiento dice que el rol Aprobador controla "la aprobacion
 * y el envio formal" al proveedor; si el proveedor pudiera bajarse una orden
 * pendiente, ese control no existiria. Y no alcanza con esconder el boton: el
 * itemId viaja en la query string.
 *
 * Cuesta una consulta a monday por descarga. En contratos se la ahorra quien no
 * tiene filtro (un admin); en ordenes se pide siempre, porque hay que mirar el
 * estado.
 */
const COLUMNAS_A_VERIFICAR = {
  FlujoContratacionSubcontratoBoard: { proveedor: "proveedores", queEs: "contrato" },
  OrdenesDeCompraMaxxaBoard: { proveedor: "proveedores", estado: "estadoDocumento", queEs: "orden" },
};

export async function verificarAccesoArchivo(sesion, boardKey, { itemId, columna: columnaPedida } = {}) {
  // La firma de quien emitio NO es un documento del proveedor: es interno, y lo
  // baja la propia app cuando el aprobador firma y el PDF se regenera. Por eso
  // no pasa por el filtro del Portal -quien aprueba una orden puede no tener el
  // Portal- sino por el permiso del OC Tracker.
  if (boardKey === "OrdenesDeCompraMaxxaBoard" && columnaPedida === "firmaEmisor") {
    requireEmisionOc(sesion);
    return;
  }

  const reglas = COLUMNAS_A_VERIFICAR[boardKey];
  if (!reglas) throw new BoardAccessError("No se pueden descargar archivos de ese tablero.");

  // Valida de paso que la sesion tenga el Portal, y que un subcontratista tenga
  // proveedor asignado.
  const filtro = filtroPortalDeSesion(sesion);

  // Un admin sin filtro no necesita que se verifique de quien es el contrato.
  // En las ordenes igual hay que traer el item, para mirar el estado.
  if (filtro.tipo === "todo" && !reglas.estado) return;

  if (!itemId) throw new BoardAccessError(`Falta el ${reglas.queEs} sobre el que se pide el archivo.`);

  const claves = [reglas.proveedor, ...(reglas.estado ? [reglas.estado] : [])];
  const ids = claves.map((c) => resolveColumnId(boardKey, c));
  let datos;
  try {
    datos = await mondayFetch(
      `query ($itemId: [ID!]) {
    ...
    console.error("[archivo] No se pudo verificar el item:", error?.message);
    throw new BoardAccessError(`No se pudo verificar tu permiso sobre este ${reglas.queEs}.`);
  }

  const valores = datos.items?.[0]?.column_values;
  if (!valores) throw new BoardAccessError(`No se encontro ese ${reglas.queEs}.`);
  const columna = (clave) => valores.find((c) => c.id === resolveColumnId(boardKey, clave));

  if (reglas.estado) {
    const estado = (columna(reglas.estado)?.text ?? "").trim().toUpperCase();
    if (estado !== "APROBADO") {
      throw new BoardAccessError("Esa orden todavia no esta aprobada.");
    }
  }

  if (filtro.tipo === "todo") return;

  if (!coincideConFiltro(proveedorDeColumna(columna(reglas.proveedor)), filtro)) {
    throw new BoardAccessError(`Ese ${reglas.queEs} no es de tu proveedor.`);
  }
}
...
/**
 * Llamar antes de deshacer una orden que no se pudo terminar de emitir
 * (op "ocRollback" en app/api/monday/board/route.js).
 *
 * NO es un borrado generico y no tiene que serlo: el endpoint ademas verifica,
 * contra monday, que la orden siga en PENDIENTE, no tenga documento adjunto y
 * se haya creado recien. Aca solo se controla quien puede pedirlo - el mismo
 * permiso que hace falta para emitirla.
 */
export function verificarAccesoRollbackOc(sesion, boardKey) {
  if (boardKey !== "OrdenesDeCompraMaxxaBoard") {
    throw new BoardAccessError("Esa operación solo existe para las órdenes de compra.");
  }
  requireEmisionOc(sesion);
}
