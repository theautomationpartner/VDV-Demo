...
export async function traerTodoElTablero(boardKey, claves) {
  ...
  const mapear = (item) => {
    ...
    for (const cv of item.column_values ?? []) {
      ...
      // Una columna de persona se sigue entregando como el TEXTO con los
      // nombres, que es lo que ya consumen las pantallas. Los ids van aparte,
      // en `<clave>Ids`: cruzar por nombre es fragil -dos personas se llaman
      // igual, alguien se renombra en monday- y "esta orden es mia" no puede
      // depender de eso.
      if (cv.persons_and_teams?.length) {
        fila[`${clave}Ids`] = cv.persons_and_teams
