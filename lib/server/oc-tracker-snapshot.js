...
import { mailsDeVinculo } from "@/lib/server/equipo-vdv";
...
const OC_COLUMNAS = [
  ...
  // Lo necesita "Mis Pendientes" para saber que ordenes espera cada persona.
  // Viene con su `aprobadorIds` al lado (ver lib/server/monday-bulk.js): el
  // cruce es por id de monday, no por nombre.
  "aprobador",
  // La misma persona, apuntando a su item en "Equipo VDV". Se agrega porque el
  // id de monday deja de existir cuando le dan de baja la licencia, y entonces
  // la bandeja quedaria vacia para todos. Ver aprobadorMails mas abajo.
  "aprobadorVdv",
  // Idem para quien emitio: es el nombre que muestran Consumo por Obra y
  // "OCs sin facturas". Se aplana a texto abajo y no se guarda aparte.
  "responsableVdv",
];
const FACTURA_COLUMNAS = [
  ...
  // Ver aplanarEncargado: el "encargado" que se muestra sale de aca.
  "encargadoVdv",
];
...
/**
 * El RESPONSABLE sale del vinculo a "Equipo VDV" y no de la columna de
 * PERSONA, que es la que muere con la licencia. Queda como texto, que es lo
 * que las pantallas ya consumen.
 *
 * La columna de PERSONA sigue de respaldo para cualquier orden que no tuviera
 * vinculo. Hoy las tienen todas -se verificaron las 474- y los nombres de las
 * dos vias son identicos, asi que esto no cambia nada visible.
 *
 * El vinculo no se guarda en el snapshot: solo se usa para esto y el snapshot
 * viaja entero a cada navegador.
 */
function aplanarResponsable(item) {
  const { responsableVdv, ...resto } = item;
  const nombres = (responsableVdv?.linkedItems ?? []).map((l) => l.name).filter(Boolean);
  return nombres.length ? { ...resto, responsable: nombres.join(", ") } : resto;
}

/**
 * El ENCARGADO de una factura, desde el vinculo a "Equipo VDV".
 *
 * Lo llena el escenario de Make "VDV - Facturas Email a Monday", copiandolo de
 * la orden de compra asociada. Ese escenario escribe las dos columnas: la de
 * PERSONA -que se queda vacia cuando le dan de baja la licencia- y la de
 * conexion. Se lee la segunda, con la primera de respaldo.
 *
 * El respaldo hace falta: de 589 facturas, 303 tienen las dos y 5 tienen solo
 * la de PERSONA. Reemplazar en vez de sumar dejaria esas 5 sin encargado.
 */
function aplanarEncargado(factura) {
  const { encargadoVdv, ...resto } = factura;
  const nombres = (encargadoVdv?.linkedItems ?? []).map((l) => l.name).filter(Boolean);
  return nombres.length ? { ...resto, encargado: nombres.join(", ") } : resto;
}

/**
 * El vinculo a "Equipo VDV" traducido a MAIL, que es lo que la bandeja puede
 * comparar sin pedir nada: su sesion ya tiene el mail de la persona.
 *
 * Si el directorio no contesta, la orden queda con `aprobadorMails` vacio y
 * "Mis Pendientes" funciona igual que antes, por el id de monday.
 */
async function agregarMailsDeAprobador(ordenes) {
  const conVinculo = ordenes.filter((oc) => oc.aprobadorVdv?.linkedItems?.length);
  if (!conVinculo.length) return ordenes;
  await Promise.all(
    conVinculo.map(async (oc) => {
      oc.aprobadorMails = await mailsDeVinculo(oc.aprobadorVdv).catch(() => []);
    }),
  );
  return ordenes;
}
