...
/**
 * Que proveedor eligio mirar un super admin en /portal-proveedor/super-admin-filter.
 *
 * Es una eleccion de la persona, no un dato de su cuenta, asi que no sale de
 * `asignacion` - hay que rescatarla de la sesion que ya estaba. AuthGate vuelve
 * a sembrar la sesion en CADA navegacion (su efecto depende de `pathname`), asi
 * que sin esto el filtro se borraba solo al cambiar de pantalla: se elegia un
 * subcontratista, el dashboard salia bien, y al entrar a Ordenes de Compra ya
 * aparecian las de todos.
 *
 * Solo se conserva si la sesion previa era de la misma persona, para que no se
 * arrastre entre cuentas cuando alguien cierra sesion y entra otro.
 */
function conservarFiltroDeSuperAdmin(sesion) {
  if (sesion.role !== "super_admin") return sesion;

  let previa = null;
  try {
    previa = JSON.parse(localStorage.getItem("pp_session") || "null");
  } catch {
    return sesion;
  }

  if (!previa?.filterMode || previa.mondayUserId !== sesion.mondayUserId) return sesion;

  return {
    ...sesion,
    filterMode: previa.filterMode,
    filterProveedor: previa.filterProveedor ?? null,
    filterProveedorId: previa.filterProveedorId ?? null,
  };
}
...
function buildPpSession(id, email, nombre, asignacion) {
  return {
    ...
    // Que pasos del circuito de contratos da esta persona y en que obras, y si
    // es super aprobador. Lo lee lib/contratos-vb.js, que es el mismo modulo
    // que usa el guardia del servidor. Lista vacia = no aprueba ninguno, que es
    // lo normal para un subcontratista.
    pasosContrato: asignacion.appConfig?.pasosContrato ?? null,
    superAprobador: asignacion.appConfig?.superAprobador === true,
    // Forma vieja: un solo paso, todas las obras. Se sigue mandando para que
    // pasosAsignados() la pueda leer mientras queden asignaciones sin migrar.
    rolContrato: asignacion.appConfig?.rolContrato ?? null,
    adminUserId: null,
    // Antes siempre null/false aca, a diferencia de useUserManagement.js que si
    // lee estos mismos campos desde appConfig - inconsistencia entre los dos
    // esquemas de pp_session que conviven en el codigo. Hoy ningun hook de
    // datos consume allowedObras/allowedProveedores todavia (ver auditoria),
    // pero cuando se conecten no deberian quedar vacios solo por haber logueado
    // via seedAppSessionFromEmail en vez de por useUserManagement.
    allowedObras: asignacion.appConfig?.allowedObras ?? null,
    allowedProveedores: asignacion.appConfig?.allowedProveedores ?? null,
    canGrantSubAccess: asignacion.appConfig?.canGrantSubAccess ?? false,
  };
}

/**
 * Sesion del Generador de OC.
 *
 * `mondayUserId` es el dato que hace falta y que las otras dos apps no
 * necesitan: la Orden de Compra escribe columnas de PERSONA en monday
 * (Responsable y APROBADOR), y esas guardan el id de monday, no un email. Se
 * carga por persona en /admin/whitelist. Sin el, la app deja emitir nada y lo
 * dice en pantalla, en vez de crear ordenes huerfanas.
 */
function buildOgSession(id, email, nombre, asignacion) {
  return {
    role: asignacion.appRol,
    userId: `db-${id}`,
    userName: nombre || email,
    email,
    mondayUserId: asignacion.appConfig?.mondayUserId ?? null,
    // Aprueba cualquier orden, no solo las que tiene designadas. Reemplaza a la
    // regla que miraba el cargo "Gerente General" del perfil de monday.
    apruebaCualquierOrden: asignacion.appConfig?.apruebaCualquierOrden === true,
  };
}
...
export function seedAppSessionFromEmail(email, serverInfo) {
  ...
  // Si el navegador tiene bloqueado el almacenamiento del sitio, lo de abajo
  // tira una excepcion en ingles que terminaba mostrandose donde va el aviso
  // del codigo de 2FA -y quien la veia creia que se habia equivocado de
  // numero-. Se corta antes y con un mensaje que se entiende.
  if (almacenamientoBloqueado()) {
    const error = new Error(MENSAJE_ALMACENAMIENTO_BLOQUEADO);
    error.almacenamientoBloqueado = true;
    throw error;
  }
  ...
  for (const asignacion of asignaciones) {
    if (asignacion.app === 'vale-express') {
      ...
      localStorage.setItem(
        'pp_session',
        JSON.stringify(conservarFiltroDeSuperAdmin(buildPpSession(id, normalized, nombre, asignacion)))
      );
      primero ??= {
    ...
    } else if (asignacion.app === 'generador-oc') {
      localStorage.setItem('og_session', JSON.stringify(buildOgSession(id, normalized, nombre, asignacion)));
      primero ??= { app: 'generador-oc', dashboardPath: '/generador-oc' };
    }
  }
