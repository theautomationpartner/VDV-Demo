...
/**
 * A que app (o ninguna) pertenece un email de las 8 cuentas fijas. Cada cuenta
 * es de UNA sola app - lo usan tanto el sidebar (para no mostrar la seccion que
 * no te corresponde) como los login pages (para no pedir el email de nuevo ni
 * ofrecer un formulario cuando ya sabemos que esa cuenta no tiene acceso ahi).
 */
export function appForEmail(email) {
  const normalized = email?.toLowerCase().trim();
  if (!normalized) return null;
  if (VALE_EXPRESS_ACCOUNTS[normalized]) return 'vale-express';
  if (PORTAL_PROVEEDOR_ACCOUNTS[normalized]) return 'portal-proveedor';
  return null;
}
 ...
 * es una de las 8 cuentas fijas), y guarda el email global para que el sidebar
 * y los login pages sepan a que app pertenece. Devuelve a que app pertenece
 * (para poder redirigir directo al dashboard despues de loguearse), o null si
 * el email no matchea ninguna cuenta de app (ej. una cuenta que solo puede ver
 * OC Tracker).
 */
