...
/**
 * Las fechas viajan como texto ISO en el JSON y vuelven a ser Date al parsear.
 * Se exporta porque el cache que se guarda en el navegador
 * (lib/client/cache-persistente.js) tiene que revivirlas con el mismo criterio:
 * si no, al recargar la pagina las fechas quedan como texto y cualquier
 * pantalla que las formatee se rompe.
 */
...
async function callBoardApi(boardKey, op, params) {
  const res = await fetch("/api/monday/board", {
    method: "POST",
    // La cabecera de traza va aca y no en cada sitio de llamada: asi cualquier
    // operacion que abra una bitacora queda hilada en los logs sin tocar nada
    // mas. Ver lib/traza-cliente.js.
  ...
  if (!res.ok) {
    const err = new Error(json.error ?? `Error en /api/monday/board (status ${res.status})`);
    // El servidor contesto y dijo que no. Sirve para distinguirlo de "no hubo
    // respuesta": lo primero no tiene sentido reintentarlo, lo segundo si.
    err.respondioServidor = true;
...
class ItemsQueryBuilder {
  constructor(boardKey) {
    ...
    this._orderBy = null;
    this._subBoardKey = null;
    this._subColumns = [];
  }
  /** Trae tambien los subelementos de cada item, en la misma consulta. */
  withSubItems(subBoardKey, columns = []) {
    this._subBoardKey = subBoardKey;
    this._subColumns = columns;
    return this;
  }
  /** Orden del listado. `column` acepta "createdAt" o una clave del schema. */
  orderBy({ column, direction = "desc" } = {}) {
    this._orderBy = column ? { column, direction } : null;
    return this;
  }
  withColumns(columns) {
  ...
  async execute() {
    return callBoardApi(this.boardKey, "items", {
      ...
      subBoardKey: this._subBoardKey,
      subColumns: this._subColumns,
    });
  }
}
...
class ItemMutator {
  ...
  /**
   * `reemplazar` deja la columna con este archivo y nada mas. Las columnas file
   * de monday acumulan: sin esto, la OC terminaba con el PDF emitido y el
   * aprobado juntos y no se sabia cual valia.
   */
  async uploadFile({ columnId, file, reemplazar = false }) {
...
/**
 * Lectura de un item puntual, con sus columnas y opcionalmente sus subelementos.
 * Los subelementos viven en OTRO tablero (el de subelementos), asi que sus
 * columnas se resuelven contra `subBoardKey`.
 */
class ItemReader {
  constructor(boardKey, itemId) {
    this.boardKey = boardKey;
  ...
  async execute() {
    return callBoardApi(this.boardKey, "item", {
      itemId: this.itemId,
      columns: this._columns,
      subBoardKey: this._subBoardKey,
      subColumns: this._subColumns,
...
class BoardBase {
  ...
  /** Lectura de un item por id (board.item(id) devuelve el mutador, no un lector). */
  get(id) {
    return new ItemReader(this.boardKey, id);
  }
  /** Los subelementos de un item, con las columnas del tablero de subelementos. */
  async subitems(itemId, subBoardKey, columns = []) {
    const r = await callBoardApi(this.boardKey, "subitems", { itemId, subBoardKey, columns });
    return r?.subitems ?? [];
  }
  /** Crea un subelemento colgando de `parentItemId`. */
  async createSubitem({ parentItemId, subBoardKey, name, values = {} }) {
    return callBoardApi(this.boardKey, "subitemCreate", { parentItemId, subBoardKey, name, values });
  }
  /** Notificacion de monday sobre un item (ej. avisarle al aprobador de una OC). */
  async notify({ userId, itemId, text }) {
    return callBoardApi(this.boardKey, "notify", { userId, itemId, text });
  }
  /**
   * Deshace una orden de compra que quedo a medio emitir, para que su numero
   * vuelva a estar disponible. El servidor solo lo acepta si la orden sigue en
   * PENDIENTE, no tiene documento y se creo recien.
   */
  async ocRollback(itemId) {
    return callBoardApi(this.boardKey, "ocRollback", { itemId });
  }
  get users() {
...
export const SubelementosDeOrdenesDeCompraMaxxaBoard = defineBoard("SubelementosDeOrdenesDeCompraMaxxaBoard");
export const EquivalenciasDeMaterialesBoard = defineBoard("EquivalenciasDeMaterialesBoard");
export const EquipoVdvBoard = defineBoard("EquipoVdvBoard");
