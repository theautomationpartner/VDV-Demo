...
/**
 * Puede quedar como aprobador de SUS PROPIAS ordenes.
 *
 * Por defecto no: la lista de aprobadores excluye a quien emite, para que nadie
 * cierre solo su propio circuito. Pero el cliente emite el 89% de las ordenes
 * desde una sola cuenta, asi que exigir siempre a un tercero le agrega una
 * espera a casi todas.
 *
 * La contrapartida es que sigan siendo DOS momentos: firma al emitir, la orden
 * queda PENDIENTE, y despues entra al historial y firma como aprobador. Nunca
 * las dos en la misma pantalla, que era el problema -en la OC 2203 las dos
 * firmas salieron el mismo minuto y el aprobador nunca la vio.
 */
export function apruebaSusOrdenes(asignacion) {
  return asignacion?.appConfig?.apruebaSusOrdenes === true && puedeAprobarOc(asignacion?.appRol);
}
