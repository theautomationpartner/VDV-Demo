...
export const BOARD_SCHEMAS = {
  OrdenesDeCompraMaxxaBoard: {
    ...
    columns: {
      ...
      // El mismo dato que `responsable` y `aprobador`, pero apuntando al item
      // de la persona en el tablero "Equipo VDV" en vez de a su usuario de
      // monday. Se escriben LAS DOS a la vez a proposito: monday cobra por
      // usuario, y el plan es dejar de depender de que la persona tenga
      // licencia. Mientras dure la transicion la columna vieja sigue siendo la
      // que manda -es la que leen el filtro de permisos y el PDF-, asi que si
      // el vinculo nuevo falla no pasa nada. Ver lib/generador-oc/equipo-vdv.js.
      responsableVdv: "board_relation_mm799zv6",
      aprobadorVdv: "board_relation_mm79rmqm",
      // La firma dibujada por quien emitio. Al aprobar, el PDF se rearma desde
      // cero y ese dibujo no estaba guardado en ningun lado: el documento
      // aprobado salia con el recuadro de quien emite vacio. Se guarda aca para
      // poder volver a estamparlo. Ver components/generador-oc/AprobarOcDialog.
      firmaEmisor: "file_mm70ezxs",
      // El PDF tal como se emitio, antes de que el aprobador firme. DOC OC
      // guarda SOLO el documento vigente: al aprobar se reemplaza por el
      // definitivo, porque tener los dos juntos confundia sobre cual vale.
      // Este queda como respaldo: comparar el emitido con el aprobado fue lo
      // unico que permitio probar que la OC 2201 habia salido sin una linea.
      docOcEmision: "file_mm706884",
  ...
  FacturasIaBoard: {
    ...
    columns: {
      ...
      // La misma persona apuntando a su ficha en "Equipo VDV". La escribe el
      // escenario de Make "VDV - Facturas Email a Monday" al lado de la de
      // PERSONA, que se queda vacia cuando le dan de baja la licencia.
      encargadoVdv: "board_relation_mm79249w",
    },
  },
  ...
  ProveedoresBoard: {
    ...
    columns: {
      ...
      direccionRepLegal: "short_texti1o6j86b",
      mailRepresentanteLegal: "emailzvj8lk17",
      correoRepLegal: "email_mkzd3wht",
  ...
  SubelementosDeOrdenesDeCompraMaxxaBoard: {
    ...
    columns: {
      // El texto completo de la linea cuando no entra en el nombre. monday
      // limita el nombre de un item a 255 caracteres y ahi viaja la linea
      // entera, asi que una descripcion larga hacia que RECHAZARA la linea:
      // pasó con la OC 2201 y se perdieron $8.008.000. Ver linea-oc.js.
      descripcionCompleta: "long_text_mm70zd4r",
  ...
  /**
   * El directorio del equipo: un item por persona, con su mail.
   *
   * Existe porque monday cobra por usuario y VDV necesita dar de baja gente sin
   * que los tableros queden diciendo "Miembro eliminado". Cada columna de
   * PERSONA del proyecto tiene al lado una columna de CONEXION que apunta aca,
   * y ese vinculo no depende de que la persona tenga licencia.
   *
   * La app lo usa solo para traducir un mail al id de su item. El cruce va por
   * MAIL y no por un id guardado a mano: asi una persona nueva funciona sin que
   * nadie se acuerde de cargarle nada.
   */
  EquipoVdvBoard: {
    boardIdEnv: "MONDAY_BOARD_EQUIPO_VDV",
    columns: {
      mail: "email_mm701kch",
      cargo: "dropdown_mm79z79v",
      telefono: "phone_mm7014ay",
      estado: "color_mm70te8n",
    },
  },
};
