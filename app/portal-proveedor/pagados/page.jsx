...
import { ArrowLeft, CheckCircle2, Building2, ChevronDown, ChevronUp, ArrowUpDown, ArrowUp, ArrowDown, Search, X } from 'lucide-react';
...
export default function PagadosPage() {
  ...
  const { obraGroups, totalMonto, totalCount } = useMemo(() => {
    ...
    return { obraGroups: groups, totalMonto: pagados.reduce((s, i) => s + (parseFloat(i.monto) || 0), 0), totalCount: pagados.length };
  }, [items, searchTerm]);
        ...
        <div className="p-3 md:p-6 space-y-4 md:space-y-6 max-w-[1400px] pb-20 md:pb-6">
          <div className="grid gap-3 grid-cols-2 md:grid-cols-3">
            {loading ? Array.from({ length: 3 }).map((_, i) => <Card key={`s-${i}`} className="p-3 md:p-4"><Skeleton className="h-14" /></Card>) : (
              <>
                <Card className="p-3 md:p-4 border-border border-l-2 border-l-green-500"><p className="text-[10px] md:text-xs text-green-400">Total Pagado</p><p className="text-sm md:text-2xl font-bold tabular-nums mt-1 text-green-400 break-all">{fmt(totalMonto)}</p></Card>
          ...
          {loading ? <Card className="p-6 border-border"><Skeleton className="h-64" /></Card> : obraGroups.length === 0 ? (
            <Card className="p-8 border-border"><p className="text-center text-muted-foreground text-sm">{searchTerm ? 'No se encontraron resultados' : 'No hay pagos con estado Pagado'}</p></Card>
          ) : obraGroups.map((group) => (
            <Card key={group.obra} className="border-border overflow-hidden">
              <button onClick={() => toggle(group.obra)} className="w-full p-3 md:p-4 border-b border-border flex items-center justify-between bg-green-950/15 active:bg-green-950/25">
                <div className="flex items-center gap-2.5 min-w-0"><Building2 className="w-4 h-4 text-green-400 shrink-0" /><h2 className="text-sm font-semibold text-foreground truncate">{group.obra}</h2><Badge variant="secondary" className="text-[10px] h-5 shrink-0">{group.items.length}</Badge></div>
                <div className="flex items-center gap-2 shrink-0 ml-2"><span className="text-xs md:text-sm font-semibold tabular-nums text-green-400">{fmt(group.totalMonto)}</span>{expandedObras[group.obra] ? <ChevronUp className="w-4 h-4 text-muted-foreground" /> : <ChevronDown className="w-4 h-4 text-muted-foreground" />}</div>
              </button>
                ...
                <>
                  <div className="md:hidden">
                    ...
                    <ScrollArea className="max-h-[500px]">
                      {sortItems(group.items).map((p) => <PaymentCard key={p.id} pago={p} formatCurrency={fmt} formatDate={fmtDate} isPagado />)}
                    </ScrollArea>
                  </div>
